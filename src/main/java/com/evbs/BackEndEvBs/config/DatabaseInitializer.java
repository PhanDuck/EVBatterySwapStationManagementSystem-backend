...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    public DatabaseInitializer(UserRepository userRepository, StationRepository stationRepository,
                               ...
                               TicketResponseRepository ticketResponseRepository, BatteryTypeRepository batteryTypeRepository) {
    ...
    private void createSampleData() {
        ...
        // 5. Tạo Vehicles
        List<Vehicle> vehicles = createVehicles(users, batteryTypes);
    ...
    private List<BatteryType> createBatteryTypes() {
        return List.of(
                createBatteryType("VinFast 48V-20Ah", "Pin VinFast 48V 20Ah cho xe máy điện", 48.0, 1.44, 8.5, "300×150×180mm"),
                createBatteryType("Yadea 60V-32Ah", "Pin Yadea 60V 32Ah cho xe máy điện", 60.0, 1.92, 10.2, "320×160×200mm"),
                createBatteryType("Pega 72V-28Ah", "Pin Pega 72V 28Ah cho xe máy điện", 72.0, 2.02, 11.5, "330×170×190mm"),
                createBatteryType("Dibao 60V-35Ah", "Pin Dibao 60V 35Ah cho xe máy điện", 60.0, 2.10, 10.8, "325×165×195mm"),
                createBatteryType("VinFast 72V-30Ah", "Pin VinFast 72V 30Ah cho xe máy điện cao cấp", 72.0, 2.16, 12.0, "340×175×205mm")
        );
    }

    private BatteryType createBatteryType(String name, String description, Double voltage, Double capacity, Double weight, String dimensions) {
        BatteryType type = new BatteryType();
        type.setName(name);
        type.setDescription(description);
        type.setVoltage(voltage);
    ...
    private List<Station> createStations(List<BatteryType> batteryTypes) {
        return List.of(
                ...
                createStation("Trạm Đổi Pin Bình Thạnh", "101 Đường Xô Viết Nghệ Tĩnh, Quận Bình Thạnh, TP.HCM", "TP.HCM", "Quận Bình Thạnh", 15, "0904567890", 10.8015, 106.7181, Station.Status.ACTIVE, batteryTypes.get(3)),
                createStation("Trạm Đổi Pin Thủ Đức", "202 Đường Võ Văn Ngân, TP Thủ Đức, TP.HCM", "TP.HCM", "TP Thủ Đức", 15, "0905678901", 10.8494, 106.7719, Station.Status.ACTIVE, batteryTypes.get(4)),
                createStation("Trạm Đổi Pin Hoàn Kiếm", "25 Phố Hàng Khay, Hoàn Kiếm, Hà Nội", "Hà Nội", "Hoàn Kiếm", 15, "0311112222", 21.0285, 105.8542, Station.Status.ACTIVE, batteryTypes.get(0)),
                createStation("Trạm Đổi Pin Cầu Giấy", "88 Đường Cầu Giấy, Cầu Giấy, Hà Nội", "Hà Nội", "Cầu Giấy", 15, "0333334444", 21.0314, 105.7969, Station.Status.ACTIVE, batteryTypes.get(1)),
                createStation("Trạm Đổi Pin Ba Đình", "156 Phố Nguyễn Thái Học, Ba Đình, Hà Nội", "Hà Nội", "Ba Đình", 15, "0355556666", 21.0364, 105.8325, Station.Status.ACTIVE, batteryTypes.get(2)),
                createStation("Trạm Đổi Pin Đống Đa", "45 Phố Láng, Đống Đa, Hà Nội", "Hà Nội", "Đống Đa", 15, "0377778888", 21.0136, 105.8270, Station.Status.ACTIVE, batteryTypes.get(3)),
                createStation("Trạm Đổi Pin Long Biên", "222 Đường Nguyễn Văn Cừ, Long Biên, Hà Nội", "Hà Nội", "Long Biên", 15, "0399990000", 21.0358, 105.8842, Station.Status.ACTIVE, batteryTypes.get(4))
        );
    }
    ...
    private List<Battery> createBatteries(List<Station> stations, List<BatteryType> batteryTypes) {
        ...
        // Mỗi trạm có 15 pin, mỗi pin có battery type tương ứng với trạm
        for (Station station : stations) {
            for (int i = 0; i < 15; i++) {
                ...
                batteries.add(createBattery(
                        station.getBatteryType().getName(),
                        new BigDecimal("1.44"),
                        health,
                        status,
                        station,
                        station.getBatteryType(),
        ...
        for (int i = 0; i < 5; i++) {
            ...
            batteries.add(createUnassignedBattery(
                    "Available Battery " + (i + 1),
                    new BigDecimal("2.00"),
                    health,
                    Battery.Status.AVAILABLE,
                    batteryTypes.get(i % batteryTypes.size())
            ));
        }
    ...
    private Battery createBattery(String model, BigDecimal capacity, BigDecimal stateOfHealth, Battery.Status status, Station station, BatteryType batteryType, LocalDate manufactureDate, LocalDate lastMaintenanceDate) {
        ...
        battery.setManufactureDate(manufactureDate);
        battery.setLastMaintenanceDate(lastMaintenanceDate);
        battery.setUsageCount((int)(Math.random() * 50)); // Số lần sử dụng ngẫu nhiên 0-49
        return battery;
    }
    ...
    private Battery createUnassignedBattery(String model, BigDecimal capacity, BigDecimal stateOfHealth, Battery.Status status, BatteryType batteryType) {
        ...
        battery.setBatteryType(batteryType);
        battery.setManufactureDate(LocalDate.now().minusMonths(3));
        battery.setLastMaintenanceDate(LocalDate.now().minusMonths(1));
        battery.setUsageCount((int)(Math.random() * 30));
        return battery;
    }

    private List<Vehicle> createVehicles(List<User> users, List<BatteryType> batteryTypes) {
        List<User> drivers = users.stream().filter(u -> u.getRole() == User.Role.DRIVER).toList();
        return List.of(
                createVehicle("VFMOTO2024001234", "59-A1 123.45", "VinFast Klara A2", drivers.get(0), batteryTypes.get(0)),
                createVehicle("VFMOTO2024001235", "51-F1 456.78", "VinFast Impes", drivers.get(1), batteryTypes.get(1)),
                createVehicle("YADEA2024001236", "59-B2 789.01", "Yadea Xmen Neo", drivers.get(2), batteryTypes.get(2)),
                createVehicle("PEGA2024001237", "51-G1 234.56", "Pega NewTech", drivers.get(3), batteryTypes.get(3)),
                createVehicle("DIBAO2024001238", "59-C3 567.89", "Dibao Angelina", drivers.get(4), batteryTypes.get(4))
        );
    }

    private Vehicle createVehicle(String vin, String plateNumber, String model, User driver, BatteryType batteryType) {
    ...
    private List<StationInventory> createStationInventory(List<Station> stations, List<Battery> batteries) {
        return List.of(
                ...
                createStationInventory(stations.get(1), batteries.get(3), StationInventory.Status.AVAILABLE),
                createStationInventory(stations.get(2), batteries.get(4), StationInventory.Status.AVAILABLE),
                createStationInventory(stations.get(2), batteries.get(5), StationInventory.Status.RESERVED)
        );
    }
    ...
    private List<Booking> createBookings(List<User> users, List<Vehicle> vehicles, List<Station> stations) {
        ...
        LocalDateTime baseTime = LocalDateTime.of(2025, 10, 11, 6, 0);

        return List.of(
                createBooking(drivers.get(0), vehicles.get(0), stations.get(0), baseTime.plusHours(2), Booking.Status.CONFIRMED),
                createBooking(drivers.get(1), vehicles.get(1), stations.get(1), baseTime.plusHours(16), Booking.Status.PENDING),
                createBooking(drivers.get(2), vehicles.get(2), stations.get(2), baseTime.plusHours(6), Booking.Status.CONFIRMED),
                createBooking(drivers.get(3), vehicles.get(3), stations.get(3), baseTime.plusHours(20), Booking.Status.CONFIRMED),
                createBooking(drivers.get(4), vehicles.get(4), stations.get(4), baseTime.plusHours(10), Booking.Status.PENDING)
        );
    }
